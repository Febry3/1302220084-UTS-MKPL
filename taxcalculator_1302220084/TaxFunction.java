...
public class TaxFunction {
    ...
    public static int calculateTax(Employee employee) {
        ...
        int totalNetYearlyIncome = ((employee.getMonthlySalary() + employee.getOtherMonthlyIncome()) * employee.getMonthWorkingInYear()) - employee.getAnnualDeductible();
        int totalNonTaxable = employee.getSpouse() != null ? (NON_TAXABLE_INCOME + NON_TAXABLE_ADDITION_FOR_NON_MARRIED + (numberOfChildren * ADDITION_PER_CHILD)) : NON_TAXABLE_INCOME;
        
        int tax = (int) Math.round(TAX_RATE * (totalNetYearlyIncome - totalNonTaxable));
